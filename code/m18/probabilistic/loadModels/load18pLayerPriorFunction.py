...
def returnModel(pri):
    ...
    _temp = __import__('divergenceFunctions', globals(), locals(), [hpPri.divFn], 0)
    divergence_fn = _temp.__dict__[hpPri.divFn]

    _temp = __import__('posteriorDists', globals(), locals(), [hpPri.posterior], 0)
    post = _temp.__dict__[hpPri.posterior] #remember we call this posterior
    post = post()
    ...
    m.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=hpPri.learningRate),
