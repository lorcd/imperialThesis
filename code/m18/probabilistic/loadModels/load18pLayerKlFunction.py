...
def returnModel(divergence_fn):
    ...
    _temp = __import__('posteriorDists', globals(), locals(), [hpKl.posterior], 0)
    post = _temp.__dict__[hpKl.posterior] #remember we call this posterior
    post = post()

    _temp = __import__('priorDists', globals(), locals(), [hpKl.prior], 0)
    pri = _temp.__dict__[hpKl.prior]
    ...
    m.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=hpKl.learningRate),
